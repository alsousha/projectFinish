...
function EditArticle() { 
  ...
  const [errors, setErrors] = useState({}); //Validations
	...
	useEffect(() => {
		setHasAccess(article)
  }, [id, currentUser.id_user]);
	
	useEffect(() => {
		if (currentUser.role === 'admin') {
			const articleObj = {
				selectedTitle: article.art_title,
				selectedText: article.art_text,
				selectedImage: article.art_img,
			}
...
// console.log(selectedData.selectedText);
	...
	return (
		<div className='container'>
			<h2 className='center mt4'>Edit article: {articleName}</h2>
