          ...
          {dataArray &&
            dataArray.map((elem, i) => (
              <div key={'art-' + i}>
                <Link
                  to={
                    isBlogAdminRoute ? `/admin/blog/${elem.id_article}` : `blog/${elem.id_article}`
                  }
                  state={{ article: elem }}
                      ...
                      <ReactQuill
                        value={
                          elem.art_text.split(' ').length > 100
                            ? elem.art_text.split(' ').slice(0, maxCountToDisplayWords).join(' ') +
                              '...'
                            : elem.art_text
                        }
                        readOnly={true}
                ...
                <button onClick={() => handleDeleteItem(elem)}>
                  <DeleteIcon />
                </button>
              </div>
            ))}
