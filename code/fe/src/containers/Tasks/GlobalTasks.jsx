		...
		if(subjects.length!==0 && templates.length !== 0){
			try {
				const res = await axios.post(`/tasks/global/all`, {
					selectedSubjects: newData.selectedSubjects.includes('all') ? subjects : newData.selectedSubjects,
					selectedTemplates: newData.selectedTemplates.includes('all') ? templates : newData.selectedTemplates,
					selectedLevels: newData.selectedLevels.includes('all') ? levels : newData.selectedLevels,
					selectedWeights: newData.selectedWeights.includes('all') ? weights : newData.selectedWeights,
				});
