...
function GroupAssigment({task, textResult, handleGoBack}) {
  ...
  return (
		<div className="task_content d-flex f-column jcsb">
					...
					<Droppable droppableId="box-1">
						{(provided) => (
							<ul className="seq__wrap group" {...provided.droppableProps} ref={provided.innerRef}>
								{data.map((item, index) => (
									<Draggable key={`item-${item.value}`} draggableId={item.value.toString()} index={index} tt={item.source}>
										{(provided) => (
											<div
					...
					<div className="answer_boxes d-flex jcsb g1">
						<div className="box w50" id="box-2">
							...
							<Droppable droppableId="box-2">
								{(provided) => (
									<div ref={provided.innerRef} className="items-container" {...provided.droppableProps}>
										{box2Data.map((item, index) => (
											<Draggable key={`itembox2-${item.value}`} draggableId={item.value.toString()} index={index} tt={item.source}>
												{(provided) => (
													<div
														ref={provided.innerRef}
														{...provided.draggableProps}
								...
								{(provided) => (
									...
									>
										{box3Data.map((item, index) => {
											return (
												<Draggable key={`itembox3-${item.value}`} draggableId={item.value.toString()} index={index} tt={item.source}>
													{(provided) => (
														<div
															ref={provided.innerRef}
															{...provided.draggableProps}
			...
			<button className={`btn_accent ${btnClass}`} onClick={handleOnCheck}>Check</button>
