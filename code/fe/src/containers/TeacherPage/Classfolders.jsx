...
function Classfolders() {
      ...
      isExistsTaskDone(itemId)
        .then((data) => {
          if (!data) {
            ...
            if (
              dataArrayFormat &&
              dataArrayFormat[activeTab].length === 1 &&
              dataArrayFormat.length === activeTab + 1
            ) {
          ...
          const msg = {
            ...
            message: !data
              ? 'Delete successful'
              : 'You can not delete this folder. There are completed tasks in it',
          };
          setMessage(msg);
          console.log(msg);
          setTimeout(() => {
  ...
  const handleSave = (itemId, id_sbj) => {
    ...
    if (Object.keys(fieldErrors).length === 0) {
      const sbjToSend =
        editedSbj === ''
          ? dataArrayFormat && dataArrayFormat[activeTab][0].subject_name
          : editedSbj;

      // console.log(sbjToSend);
      setDataArray((prevItems) =>
        prevItems.map((item) =>
          item.id_tskFolder === itemId
            ? { ...item, tskFolder_name: editedText, subject_name: sbjToSend }
            : item,
        ),
      );
  ...
  const addNewItem = async () => {
    ...
    if (dataArrayFormat && dataArrayFormat.length === 0) {
      ...
      sbjToSend =
        newSbj === ''
          ? dataArrayFormat &&
            dataArrayFormat.length !== 0 &&
            dataArrayFormat[activeTab][0].subject_name
          : newSbj;
    }

    axios
      .post(`/teacher/tskfolder`, {
        tskFolder_name: newItemName,
        id_class: id_class,
        subject_name: sbjToSend,
      })
      .then((res) => {
        const msg = {
              ...
              <div className='item_title d-flex g1'>
                ...
                {currentUser.sbjs.length > 1 && (
                  <select
                    ...
                    defaultValue={
                      dataArrayFormat && dataArrayFormat.length !== 0
                        ? dataArrayFormat[activeTab][0].subject_name
                        : ''
                    }>
                    {currentUser.sbjs &&
                      currentUser.sbjs.map((elem, i) => (
                ...
                <div key={'tabContent-' + i}>
                  <div key={item.id_tskFolder} className='folder__wrap mb2'>
                    <div className='folder big d-flex f-column'>
                      ...
                      dataArrayFormat.length > 1
                        ? dataArrayFormat[activeTab][0].subject_name
                        : dataArrayFormat[0][0].subject_name,
                    )}`}>
                        <FolderIcon />
                      </Link>
                        ...
                        <div className='folder__edit'>
                          ...
                          {currentUser.sbjs.length > 1 && (
                            <select
                              ...
                              defaultValue={dataArrayFormat[activeTab][0].subject_name}>
                              {currentUser.sbjs &&
                                currentUser.sbjs.map((elem, j) => (
                      ...
                      <div className='icons__wrap'>
                        <div className='table_icon'>
                          {editingItemId === item.id_tskFolder ? (
                            <button
                              onClick={() => handleSave(item.id_tskFolder, item.id_subject)}
                              className=''>
                              <CheckIcon />
                            </button>
                          ) : (
                            <button
                              onClick={() => handleEdit(item.id_tskFolder, item.tskFolder_name)}
                              className=''>
                              <EditIcon className='' />
                            </button>
                          )}
                        ...
                        <div className='class_delete table_icon'>
                          <button onClick={() => handleDelete(item.id_tskFolder)}>
                            <DeleteIcon />
                          </button>
                        </div>
                      </div>
