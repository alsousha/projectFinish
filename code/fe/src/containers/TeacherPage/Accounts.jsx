...
function Accounts() {
  ...
  const handleStudentsArray = async (arr) => {
    ...
    if (notFoundEmails.length > 0) {
      const msg = {
        msgClass: 'error',
        message: `Next users is not found: ${notFoundEmails.join(', ')}`,
      };
      setMessage(msg);
    }
  };
  ...
  const addNewItem = async (email, isSingleEmail) => {
    if (isSingleEmail) {
      axios
        .post(`/teacher/students/${currentUser.id_user}`, {
          student_email: email,
          id_class: id_class,
        })
        .then((res) => {
          if (res.status === 209) {
            ...
            // const tmpCat = res.data
            setAccounts((prevCats) => [...prevCats, res.data]);
  ...
  const fetchData = async () => {
    ...
    try {
      const res = await axios.post(`/teacher/${currentUser.id_user}/students`, {
        id_class: id_class,
      });
  ...
  return (
    <div className='mt4 section_accounts'>
      <div className='container'>
        ...
        <div className='add_newItem mt4 mb3'>
          <button className='link d-flex jcsb aic g1' onClick={() => setIsAddItemVisiable(true)}>
            <AddIcon />
            add new student
          </button>
        </div>
