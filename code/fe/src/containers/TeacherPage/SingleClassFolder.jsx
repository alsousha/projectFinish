...
function SingleClassFolder(props) {
  ...
  useEffect(() => {
    ...
    const checkOwnership = async () => {
      try {
        ...
        const res = await axios.post(`/teacher/folder`, {
          id_teacher: currentUser.id_user,
          id_folder: id_tskFolder,
        });
  ...
  const isNotPublishText =
    'Add/Remove Tasks and click the "Publish Tasks" button. Please note that once a folder is published, you will NOT be able to edit or delete the folder';
  return (
    <div className='mt3'>
      <div className='container'>
        ...
        <div className='folder_status d-flex aic g3 mt2'>
          ...
          <div className='d-flex g3 aic'>
            ...
            {!isFolderPublish && (
              <button className={`btn_accent ${publishBtnClass}`} onClick={handlePublishTasks}>
                Publish tasks
              </button>
            )}
          </div>
        ...
        <div className='back mt2 btn_main'>
          <button onClick={handleGoBack} className='d-flex aic g1'>
            <BackIcon />
            <span>Go Back</span>
          </button>
        </div>
          ...
          {dataArray && dataArray.length !== 0 ? (
            dataArray.map((item, i) => (
              <div key={'f-task' + item.id_task} className='arr_item d-flex f-column jcc'>
                <Task_card item={item} />
                {!isFolderPublish && (
                  <button onClick={() => handleDeleteItem(item.id_task)} className='mt1'>
                    <DeleteIcon />
                  </button>
                )}
              </div>
            ...
            {categories && categories.length > 0 ? (
              <div className='add_tasks__wrap d-flex jcsb'>
                <div className='select'>
                  <select id='cats' name='cat' onChange={handleSelectAddChange}>
                ...
                <div className='add_tasks__inner arr_wrap tasks_wrap d-flex flex-3 jcsb'>
                  {tasks && tasks.length !== 0 ? (
                    tasks.map((item, i) => (
                      <div
                        key={'add-task' + item.id_task}
                        className={`arr_item d-flex f-column ${item.className}`}>
                        <Task_card item={item} />
