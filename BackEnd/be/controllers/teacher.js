...
export const getAllTasksByClassFolder = (req, res) => {
  const token = req.cookies.access_token;
  if (!token) return res.status(401).json('Not authenticated!');

  try {
    const { id_classFolder } = req.params;

    // Query to retrieve all tasks for the given id_classFolder
    const query = `
      SELECT task.id_task, task.task_name, task.task_avatar
      FROM class
      JOIN task ON class.id_class = task.id_class
      WHERE class.id_classFolder = ?
    `;

    db.query(query, [id_classFolder], (err, result) => {
      if (err) {
        console.error("Error retrieving tasks:", err);
        return res.status(500).json("Failed to retrieve tasks.");
      }
